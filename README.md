...
### 2. Встановлення служби (одноразово)

```bash
./install.sh
```

### 3. Оновлення після змін (наприклад, після git pull)
...
Після встановлення сервіс запускається автоматично як systemd-сервіс `nutriscan.service`.
...
```bash
sudo systemctl status nutriscan
```

---

> ℹ️ За замовчуванням FastAPI запускається на `http://0.0.0.0:8000`
