...
При кожному `POST` на `http://nutriscan.radarme.com.ua/analyze/` додайте **два** HTTP-headers:
...
```bash
curl -X POST http://nutriscan.radarme.com.ua/analyze/ \
...
🌐 http://nutriscan.radarme.com.ua/users — у списку зʼявиться новий рядок з вашим іменем/аватаркою (замість маски `7Bx9zP…RtUi`).
